...
export const OtpInput = ({numberOfInputs, onChangeText}: OtpInputProps) => {
    const ref = useRef<any>([...Array<any>(numberOfInputs)].map(()=> createRef()))
  ...
  return(
      <div className={"w-full h-[70px] flex items-center justify-center gap-3 mt-5 mb-5"}>
          {
              inputBoxes?.map((_, index)=>{
                  return(
                      <input key={index} ref={ref.current[index]} onKeyUp={(event)=> handleKeyUp(event.key, index)} onChange={(event)=> handleOnChangeText(event.target.value, index)}  maxLength={1}  className={"w-[80px] h-full focus:outline-0 text-center border-[3px] border-grayColor_4 rounded-[4px]"}/>
                  )
              })
          }
      </div>
